...
>  时钟初始化，首先设置8353 timer-chip；然后初始化time counter 'ticks' 为0；
   最后设置使能端。
...
> 首先实现一个可以显示字符串的bootloader，然后实现一个可加载ELF格式文件的bootloader，实现ucore内核然后在跳转到elf的入口。


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- [x]  

> 在系统内核调用表中添加一个整形数，每次启动之后清零，每次系统调用的时候使这个数加一，就可以统计了。

如何lab1, 实现一个可显示字符串"THU LAB1"且依然能够正确加载ucore OS的bootloader？如果不能完成实现，请说明理由。
- [x]  

>  在bootmain.c文件的bootmain函数里增加一个printfmt字符串"THU LAB1"的语句。

对于ucore_lab中的labcodes/lab1，我们知道如果在qemu中执行，可能会出现各种稀奇古怪的问题，比如reboot，死机，黑屏等等。请通过qemu的分析功能来动态分析并回答lab1是如何执行并最终为什么会出现这种情况？
- [x]  

> 程序会卡死在bootmain函数的while(1)中，一直在等待。

对于ucore_lab中的labcodes/lab1,如果出现了reboot，死机，黑屏等现象，请思考设计有效的调试方法来分析常在现背后的原因。
- [x]  

>  在tools/gdbinit中增加命令，define hook-stop,x/2i $pc,end显示当前的两条汇编命令，并在其中加入break *(0x7c00) 使得模拟器在入口地址处停止，使用stepi命令查看单条汇编指令对照即可调试。
